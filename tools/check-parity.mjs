...
//   6. no Thai characters leak into a fenced ```rust code block or a code
...
// Fenced code blocks tagged ```rust — the only fence language this course
// guarantees stays English (```text fences are used for Thai-captioned
// ASCII diagrams and are intentionally not checked here).
function fencedRustBlocks(src) {
  return [...src.matchAll(/```rust\n([\s\S]*?)```/g)].map((m) => m[1]);
}
...
for (const enPath of files) {
  ...
  // 7. every fenced ```rust block is byte-identical EN vs TH (fences ARE the
  //    code in this course; TS Playground/verify-fences run the EN copy only)
  {
    const enF = fencedRustBlocks(enSrc), thF = fencedRustBlocks(thSrc);
    if (enF.length !== thF.length) {
      report(`${enPath}: rust fence count EN=${enF.length} TH=${thF.length}`);
    } else {
      enF.forEach((b, i) => { if (b !== thF[i]) report(`${enPath}: rust fence #${i} differs EN vs TH`); });
    }
  }

  // 6. no Thai characters inside a fenced ```rust block or code literal in TH file
  {
    const enFences = fencedRustBlocks(enSrc);
    fencedRustBlocks(thSrc).forEach((block, i) => {
